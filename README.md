...
- Прописать путь к исполняемому файлу wkhtmltopdf.exe в PATH используемой ОС.
- Скопировать этот путь
- Переход в папку с файлом settings.py
```
cd cash_machine/cash_machine
```
- Указать скопированный путь в поле ```WKHTMLTOPDF_LOCAL_PATH``` на 133 строчке, если он отличается от существующего.
- Переход в папку с файлом views.py
```
cd ..
cd api
```
- Открыть views.py в рабочем приложении
- Изменить в строчке №208 следующую строчку:
```
...
- После успешного сканирования QR-кода на вашем смартфоне произойдёт автоматическая переадресация на данный кассовый чек в вашем мобильном браузере по-умолчанию.
  ...
  <details>
    <summary>Пример открываемого кассового чека (<b>нажмите на эту строчку, чтобы открыть</b>):</summary>

  ![test_receipt_final.png](readme_media%2Ftest_receipt_final.png)
...
- В шкалу URL своего браузера необходимо ввести следующий адрес: ```http://158.160.48.231/api/v1/swagger-ui/#/```
- Открыть метод ```POST /cash_machine``` в блоке <b>Кассовый чек - генерация QR-кода</b>
